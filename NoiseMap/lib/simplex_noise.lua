...
local function grad3(hash_value, x, y, z)
    local h = hash_value & 15
...
local function noise3(x, y, z)
    local n0, n1, n2, n3 = 0.0, 0.0, 0.0, 0.0
    ...
    local gi0 = hash(i + hash(j + hash(k)))
    local gi1 = hash(i + i1 + hash(j + j1 + hash(k + k1)))
    local gi2 = hash(i + i2 + hash(j + j2 + hash(k + k2)))
    local gi3 = hash(i + 1 + hash(j + 1 + hash(k + 1)))
...
function SimplexNoise.new(frequency, amplitude, lacunarity, persistence)
    return setmetatable({
...
function SimplexNoise:fractal(octaves, x, y, z)
    local octave_count = math_floor((octaves or 1) + 0.5)
    if octave_count < 1 then
        octave_count = 1
    end
    ...
    for _ = 1, octave_count do
        output = output + (amplitude * noise3(x * frequency, y * frequency, z * frequency))
