...
function M.extract(art, max_colors)
  ...
  for gy = 0, grid_size - 1 do
    for gx = 0, grid_size - 1 do
      ...
      if closest_idx > 0 and closest_dist < min_dist_sq then
        pal[closest_idx][4] = pal[closest_idx][4] + 1.0
      elseif #pal < max_colors then
        pal[#pal + 1] = { r, g, b, 1.0 }
      end
  ...
  for i = 1, #pal do
    total_weight = total_weight + pal[i][4]
  end

  if total_weight <= 0.0 then
    local uniform = 1.0 / #pal
    for i = 1, #pal do
      pal[i][4] = uniform
    end
  else
    for i = 1, #pal do
      pal[i][4] = pal[i][4] / total_weight
    end
  end

  table.sort(pal, function(a, b) return a[4] > b[4] end)
...
function M.sample(pal, pos_01, flow_phase_01)
  ...
  if #pal == 1 then
    return pal[1][1], pal[1][2], pal[1][3]
  end
  ...
  for i = 1, #pal do
    ...
    if pos <= next_cum or i == #pal then
      index0 = i
      index1 = (i < #pal) and (i + 1) or 1
